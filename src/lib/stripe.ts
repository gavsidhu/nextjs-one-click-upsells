...
export const createPaymentIntent = async (paymentMethodId: string) => {
    const paymentIntent = await axios.post("http://localhost:3000/api/create-payment-intent", {
        amount: 1000,
        currency: "usd",
        name: "Joa Shmoe",
...
export const updatePaymentIntent = async (paymentIntentId: string, amount: number) => {
    const paymentIntent = await axios.post("http://localhost:3000/api/update-payment-intent", {
        paymentIntentId,
        amount
    })
...
export const confirmPaymentIntent = async (paymentIntentId: string) => {
    ...
    const result = await axios.post("http://localhost:3000/api/confirm-payment-intent", {
        paymentIntentId,
    })
