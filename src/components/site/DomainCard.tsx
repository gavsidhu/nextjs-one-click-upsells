...
interface DomainCardProps<T = Shop> {
    data: T;
}
...
export default function DomainCard({ data }: DomainCardProps) {
    const { data: valid, isValidating } = useSWR<Shop>(
        `/api/domains/check?domain=${data.custom_domain}`,
        fetcher,
    ...
    const subdomain =
        data.custom_domain && data.custom_domain.split(".").length > 2
            ? data.custom_domain.split(".")[0]
            : "";

    return (
        <div className="w-full max-w-2xl mt-10 border rounded-lg py-10">
            <div className="flex flex-col space-y-4 sm:flex-row sm:space-x-4 justify-between px-10">
                ...
                <div className="flex space-x-3">
                    <button
                        onClick={() => {
                            mutate(`/api/domain/check?domain=${data.custom_domain}`);
                        }}
                    ...
                    <button
                        onClick={async () => {
                            setRemoving(true);
                            await axios.delete(
                                `/api/domain?domain=${data.custom_domain}&siteId=${data.id}`
                            ).then((res) => {
                                setRemoving(false);
                                if (res.status === 200) {
                        ...
                        <div className="my-3 text-left">
                            ...
                            <div className="flex justify-start items-center space-x-10 bg-gray-50 p-2 rounded-md">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-mono mt-2">
                                        {recordType == "CNAME" ? `cname.tryspark.io` : `76.76.21.21`}
                                    </p>
                                </div>
                            </div>
                        </div>
